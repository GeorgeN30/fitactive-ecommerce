...
const LEVEL_CONFIG: Record<
  string,
  { label: string; cls: string; badge: string; border: string }
> = {
  ok: {
    label: "En stock",
...
export default function ReceptionistLayout() {
  ...
  const [stocks, setStocks] = useState<StockState>(
    Object.fromEntries(
      mockProducts.map((p) => [
        p.id,
        Object.fromEntries(p.sizes.map((s) => [s.size, s.stock])),
      ]),
    ),
  );
  ...
  const getStockLevel = (productId: string): keyof typeof LEVEL_CONFIG => {
    ...
    if (product && total <= Math.max(5, Math.floor(product.totalStock * 0.25)))
      return "low";
    return "ok";
  };

  const alertProducts = mockProducts.filter(
    (p) => getStockLevel(p.id) !== "ok",
  );

  const stockOk = mockProducts.filter(
    (p) => getStockLevel(p.id) === "ok",
  ).length;
  const stockLow = mockProducts.filter(
    (p) => getStockLevel(p.id) === "low",
  ).length;
  const stockCritical = mockProducts.filter(
    (p) => getStockLevel(p.id) === "critical",
  ).length;
  const stockOut = mockProducts.filter(
    (p) => getStockLevel(p.id) === "out",
  ).length;
  ...
  const handleRestock = (productId: string) => {
    ...
    setStocks((prev) => ({
      ...
      [productId]: {
        ...prev[productId],
        [firstSize]: (prev[productId]?.[firstSize] || 0) + qty,
      },
    }));
    ...
    <div className="flex flex-col h-full bg-[#0A0A0A] text-white w-64 flex-shrink-0">
      <div className="p-6 border-b border-white/10">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-[10px] text-amber-400 uppercase tracking-wider mt-1">
              Inventario
            </div>
          </div>
        </div>
      ...
      <nav className="flex-1 p-4 space-y-1 overflow-y-auto custom-scrollbar">
        {navItems.map((item) => (
          ...
          >
            <i
              className={`fa-solid ${item.icon} w-5 h-5 ${section === item.key ? "text-[#0A0A0A]" : ""}`}
            />
            {item.label}
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div
            key={kpi.label}
            className={`${kpi.bg} rounded-2xl p-5 border border-white`}
          >
            ...
            <p className={`text-sm font-medium mt-1 ${kpi.text}`}>
              {kpi.label}
            </p>
          </div>
        ))}
          ...
          <div className="space-y-2">
            {alertProducts.slice(0, 4).map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  className={`flex items-center gap-3 p-3 rounded-xl bg-gray-50 ${cfg.border}`}
                >
                  <i
                    className={`fa-solid ${cfg.badge} text-lg ${level === "ok" ? "text-green-500" : level === "low" ? "text-amber-500" : level === "critical" ? "text-orange-500" : "text-red-500"}`}
                  />
                  <img
                    src={p.imageUrl}
                    alt={p.name}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">
                      {total} unidades · SKU {p.sku}
                    </p>
                  </div>
                  <span
                    className={`text-xs font-bold px-2 py-1 rounded-full ${cfg.cls}`}
                  >
                    {cfg.label}
                  </span>
                  <button
                    onClick={() => handleRestock(p.id)}
                    className="text-xs font-semibold px-3 py-1.5 bg-[#0A0A0A] text-white rounded-lg hover:bg-gray-800 transition-all whitespace-nowrap"
                  >
                    Reabastecer
                  </button>
                </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
              ...
              <div key={s.label}>
                ...
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${(s.count / mockProducts.length) * 100}%`,
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100">
          ...
          <div className="space-y-2">
            {[
              {
                label: `Ver ${alertProducts.length} alertas activas`,
                desc: "Productos por debajo del mínimo",
                icon: "fa-triangle-exclamation",
                section: "alerts" as Section,
              },
              {
                label: "Registrar reabastecimiento",
                desc: "Actualizar stock de productos",
                icon: "fa-box-open",
                section: "restocking" as Section,
              },
              {
                label: "Ver inventario completo",
                desc: `${mockProducts.length} productos registrados`,
                icon: "fa-list",
            ...
            <thead>
              <tr className="border-b border-gray-100">
                {[
                  ...
                  <th
                    key={h}
                    className="px-4 py-3 text-xs font-bold text-gray-400 uppercase tracking-wider text-left"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {filteredProducts.map((p) => {
                ...
                return (
                  <tr
                    key={p.id}
                    className={`border-b border-gray-50 hover:bg-gray-50 transition-colors ${level !== "ok" ? "bg-amber-50/30" : ""}`}
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {p.sizes.map((s) => (
                          <span
                            key={s.size}
                            className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${(stocks[p.id]?.[s.size] || 0) === 0 ? "bg-red-100 text-red-600" : (stocks[p.id]?.[s.size] || 0) <= 2 ? "bg-amber-100 text-amber-700" : "bg-green-100 text-green-700"}`}
                          >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-xs font-semibold px-2 py-1 rounded-full ${cfg.cls}`}
                      >
                        {cfg.label}
                      </span>
                    </td>
      ...
      {alertProducts.length === 0 ? (
        ...
        alertProducts.map((p) => {
          ...
          return (
            <div
              key={p.id}
              className={`bg-white rounded-2xl p-5 border border-gray-100 ${cfg.border}`}
            >
              <div className="flex items-center gap-4">
                <i
                  className={`fa-solid ${cfg.badge} text-2xl ${level === "low" ? "text-amber-500" : level === "critical" ? "text-orange-500" : "text-red-500"}`}
                />
                <img
                  src={p.imageUrl}
                  alt={p.name}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span
                      className={`text-xs font-bold px-2 py-0.5 rounded-full ${cfg.cls}`}
                    >
                      {cfg.label}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-4 mt-2 text-sm">
                    ...
                    <div>
                      ...
                      <span className="font-bold text-gray-900">{p.sku}</span>
                    </div>
                  </div>
                </div>
                <div className="flex flex-col gap-2">
                  ...
                  >
                    {restocked.has(p.id) ? (
                      <>
                        <i className="fa-solid fa-check" /> Actualizado
                      </>
                    ) : (
                      <>
                        <i className="fa-solid fa-rotate" /> Reabastecer
                      </>
                    )}
                  </button>
                  <button
                    onClick={() => setSection("inventory")}
                    className="px-4 py-2 rounded-xl border border-gray-200 text-gray-600 font-semibold text-sm hover:bg-gray-50"
                  >
                    Ver producto
                  </button>
                </div>
              ...
              {restocked.has(p.id) && (
                <div className="mt-3 bg-green-50 border border-green-200 rounded-xl px-4 py-2.5 flex items-center gap-2">
                  ...
                  <span className="text-green-700 text-sm font-semibold">
                    Inventario actualizado · Stock normalizado
                  </span>
                </div>
              )}
            </div>
    ...
    <div className="space-y-4">
      <div className="bg-white rounded-2xl p-6 border border-gray-100 mb-6">
        ...
        <p className="text-sm text-gray-500">
          Selecciona el producto y la cantidad a agregar al inventario.
        </p>
      </div>
      {mockProducts.map((p) => {
        const level = getStockLevel(p.id) as keyof typeof LEVEL_CONFIG;
        ...
        return (
          <div
            key={p.id}
            className={`bg-white rounded-2xl p-5 border border-gray-100 ${level !== "ok" ? "border-l-4 border-amber-400" : ""}`}
          >
            <div className="flex items-center gap-4">
              <img
                src={p.imageUrl}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-3 mt-1 text-xs text-gray-500">
                  <span>
                    Stock: <strong className="text-gray-900">{total}</strong>
                  </span>
                  <span>
                    Total: <strong>{p.totalStock}</strong>
                  </span>
                  <span
                    className={`font-semibold px-2 py-0.5 rounded-full ${priorityCls}`}
                  >
                    Prioridad: {priority}
                  </span>
                </div>
  ...
  const renderNotifications = () => (
    <div className="space-y-3 max-w-2xl">
      <div className="bg-white rounded-2xl p-6 border border-gray-100">
        ...
        <p className="text-sm text-gray-500">
          Alertas de stock y actividades recientes.
        </p>
      </div>
      {alertProducts.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 border border-gray-100 text-center">
        ...
        alertProducts.map((p) => {
          ...
          return (
            <div
              key={p.id}
              className="bg-white rounded-2xl p-4 border border-gray-100 border-l-4 border-l-amber-400"
            >
              <div className="flex items-start gap-3">
                <div
                  className={`text-lg flex-shrink-0 ${level === "out" ? "text-red-500" : level === "critical" ? "text-orange-500" : "text-amber-500"}`}
                >
                  <i className="fa-solid fa-triangle-exclamation" />
                </div>
                <div className="flex-1">
                  <p className="text-sm font-semibold text-gray-900">
                    Stock {LEVEL_CONFIG[level].label.toLowerCase()} · {p.name}
                  </p>
                  <p className="text-xs text-gray-500 mt-0.5">
                    {total} unidades restantes · SKU {p.sku}
                  </p>
  ...
  const renderConfig = () => {
    const items = [
      {
        key: "a" as const,
        label: "Alertas automáticas de stock bajo",
        desc: "Notificar cuando el stock alcance el mínimo",
      },
      {
        key: "b" as const,
        label: "Notificaciones de agotados",
        desc: "Aviso inmediato cuando un producto llega a 0",
      },
      {
        key: "c" as const,
        label: "Reporte semanal de inventario",
        desc: "Resumen cada lunes a las 9:00 AM",
      },
    ];
    return (
        ...
        <div className="space-y-4">
          {items.map((item) => (
            ...
            >
              ...
              <button
                onClick={() =>
                  setConfigToggles((t) => ({ ...t, [item.key]: !t[item.key] }))
                }
                className={`w-12 h-6 rounded-full relative transition-colors ${configToggles[item.key] ? "bg-amber-400" : "bg-gray-200"}`}
              >
                <span
                  className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow-sm transition-all ${configToggles[item.key] ? "right-1" : "left-1"}`}
                />
              </button>
            </div>
          ))}
          ...
          <div className="flex items-center gap-3">
            {alertProducts.length > 0 && (
              <div className="hidden sm:flex items-center gap-1.5 bg-amber-50 border border-amber-200 rounded-xl px-3 py-1.5 text-xs font-semibold text-amber-700">
                ...
                {alertProducts.length} alertas activas
              </div>
            )}
            <button
