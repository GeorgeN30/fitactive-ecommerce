...
export type AdminTab = "dashboard" | "products" | "inventory" | "orders" | "customers" | "returns" | "metrics" | "settings" | "notifications";
...
export default function AdminSidebar({ activeTab, onTabChange, mobileOpen = false, onClose = () => undefined }: AdminSidebarProps) {
  const { logout } = useAuth();

  return (
    <>
      {mobileOpen && <div className="fixed inset-0 z-40 bg-black/50 lg:hidden" onClick={onClose} />}
      <aside className={`${mobileOpen ? "fixed inset-y-0 left-0 z-50 flex" : "hidden lg:flex"} w-64 flex-col h-full flex-shrink-0 bg-[#0A0A0A] text-white`}>
      <div className="p-6 border-b border-white/10">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-[10px] text-gray-400 uppercase tracking-wider mt-1">Admin Panel</div>
          </div>
        </div>
      </div>
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all text-left ${
                isActive ? "bg-[#00E87A] text-[#0A0A0A] font-bold" : "text-gray-400 hover:text-white hover:bg-white/5"
              }`}
            >
              <i className={`${item.icon} w-5 h-5 ${isActive ? "text-[#0A0A0A]" : ""}`} />
