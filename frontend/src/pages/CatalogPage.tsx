...
const GENDER_OPTIONS = ["Todos", "Hombre", "Mujer"];
...
export default function CatalogPage() {
  ...
  const productsForGender = selectedGender === "Todos"
    ? products
    : products.filter((p) => genderLabel(p.genero) === selectedGender);

  const categoryOptions = ["Todas", ...Array.from(new Set(productsForGender.map(p => p.categoria).filter((c): c is string => Boolean(c))))];
  const brandOptions = ["Todas", ...Array.from(new Set(products.map(p => p.marca).filter((b): b is string => Boolean(b))))];
  ...
  }, [inStock, products, selectedGender, selectedSize, sort, selectedCategory, selectedBrand, searchQuery]);
    ...
    <AppLayout>
      <div className="bg-[#f8f9fa] dark:bg-brand-dark-bg min-h-screen py-8 font-sans text-gray-900 dark:text-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-8"><h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 dark:text-white tracking-tight">Catálogo Deportivo</h1></div>
            ...
            <div className="flex-1">
              <div className="flex flex-col sm:flex-row justify-between items-center bg-white dark:bg-brand-card-dark rounded-xl p-4 mb-6 shadow-sm border border-gray-100 dark:border-gray-800">
                <p className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-4 sm:mb-0">
                  Mostrando <span className="font-bold text-black dark:text-white">{visibleProducts.length}</span> de {filteredProducts.length} productos
                  {searchQuery && (
                    <span className="ml-2 inline-block px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide bg-brand-green/10 text-brand-green rounded">
                      Búsqueda: "{searchQuery}"
                    </span>
                  )}
                </p>
                <div className="flex items-center gap-2"><span className="text-sm font-medium text-gray-500 dark:text-gray-400">Ordenar:</span><select value={sort} onChange={(event) => setSort(event.target.value)} className="text-sm font-bold bg-transparent dark:bg-brand-card-dark text-gray-900 dark:text-white border-none focus:ring-0 cursor-pointer"><option value="featured">Más destacados</option><option value="low">Precio menor</option><option value="high">Precio mayor</option></select></div>
              </div>
