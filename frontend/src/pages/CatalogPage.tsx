...
export default function CatalogPage() {
  ...
  const filteredProducts = useMemo(() => {
    const result = products.filter((product) => {
      ...
      return (selectedGender === "Todos" || genderLabel(product.genero) === selectedGender) && hasSize && hasStock;
    });
  ...
  }, [inStock, products, selectedGender, selectedSize, sort]);
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex-1">
              ...
              {loading ? <div className="p-12 text-center text-gray-400">Cargando productos desde la base de datos…</div> : error ? <div className="p-12 text-center text-red-500 font-bold">{error}</div> : visibleProducts.length === 0 ? <div className="p-12 text-center bg-white dark:bg-brand-card-dark rounded-2xl text-gray-500">No encontramos productos con esos filtros.</div> : <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{visibleProducts.map((product) => {
                ...
                return <div key={product.id} className="bg-white dark:bg-brand-card-dark rounded-2xl p-4 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-gray-100 dark:border-gray-800 flex flex-col"><div className="relative bg-[#f4f5f7] dark:bg-gray-800 rounded-xl aspect-[4/5] mb-4 flex items-center justify-center overflow-hidden"><span className={`absolute top-3 left-3 px-2 py-1 text-[8px] font-black uppercase tracking-wider text-white rounded shadow-sm z-10 ${product.totalStock <= 5 ? "bg-red-500" : "bg-gray-900/70 dark:bg-black/50"}`}>{stockLabel}</span>{pricing.discount > 0 && <span className="absolute bottom-3 left-3 px-2 py-1 text-[10px] font-black bg-brand-green text-black rounded z-10">-{pricing.discount}%</span>}<button type="button" onClick={() => toggleFavorite({ id: product.id, cat: product.categoria || "General", name: product.nombre, price: pricing.price, img: image })} className="absolute top-3 right-3 p-1.5 bg-white dark:bg-gray-700 rounded-full shadow-md hover:scale-110 transition-all z-10"><span className={isFavorite(product.id) ? "text-red-500" : "text-gray-400"}>♥</span></button><Link to={`/producto/${product.id}`} className="w-full h-full"><img src={image} alt={product.nombre} className="object-cover w-full h-full mix-blend-multiply dark:mix-blend-normal hover:scale-105 transition-transform duration-500" /></Link></div><div className="mb-1 text-[10px] text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider">{product.categoria || "General"}</div><h3 className="font-extrabold text-sm text-gray-900 dark:text-white leading-tight mb-2 truncate"><Link to={`/producto/${product.id}`}>{product.nombre}</Link></h3><div className="font-black text-lg mb-3 dark:text-gray-200">{pricing.discount > 0 && <span className="text-sm text-gray-400 line-through mr-2">S/ {product.precio.toFixed(2)}</span>}S/ {pricing.price.toFixed(2)}</div><div className="flex gap-1 mb-4 mt-auto">{product.tallas.filter((size) => size.stock > 0).map((size) => <span key={size.id} className="w-7 h-5 flex items-center justify-center bg-gray-100 dark:bg-gray-700 rounded text-[9px] font-bold text-gray-600 dark:text-gray-300">{size.talla}</span>)}</div><div className="flex gap-2"><Link to={`/producto/${product.id}`} className="flex-1 py-2 bg-gray-100 dark:bg-gray-700 text-xs font-bold text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-200 dark:hover:bg-gray-600 transition text-center">Ver detalle</Link><Link to={`/probador-virtual?producto=${product.id}`} className="flex-1 py-2 bg-white dark:bg-transparent border border-brand-green text-brand-green text-xs font-bold rounded-md hover:bg-brand-green hover:text-black transition text-center">Probar AR</Link></div></div>;
              })}</div>}

              {pageCount > 1 && <div className="flex justify-center mt-10 gap-2"><button type="button" onClick={() => setCurrentPage(Math.max(1, page - 1))} className="w-8 h-8 rounded-md bg-white dark:bg-brand-card-dark border text-gray-500">‹</button>{Array.from({ length: pageCount }, (_, index) => index + 1).map((item) => <button type="button" key={item} onClick={() => setCurrentPage(item)} className={`w-8 h-8 rounded-md shadow-sm transition font-medium ${page === item ? "bg-brand-green text-black font-bold" : "bg-white dark:bg-brand-card-dark border text-gray-700 dark:text-gray-300"}`}>{item}</button>)}<button type="button" onClick={() => setCurrentPage(Math.min(pageCount, page + 1))} className="w-8 h-8 rounded-md bg-white dark:bg-brand-card-dark border text-gray-500">›</button></div>}
            </div>
          </div>
        </div>
      </div>
