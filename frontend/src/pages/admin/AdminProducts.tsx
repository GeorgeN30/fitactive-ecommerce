...
export default function AdminProducts() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const stockPerSize = Math.floor(
      Number(formData.stock) / Math.max(formData.sizes.length, 1),
    );
    const productData = {
      name: formData.name,
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs font-medium text-slate-500 mt-0.5">
            Administra el catalogo de productos FitLook
          </p>
        </div>
        <button
                    ...
                    <div className="flex gap-1 flex-wrap">
                      {p.sizes
                        ...
                        .map((s) => (
                          <span
                            key={s.size}
                            className="px-1.5 py-0.5 bg-slate-100 rounded text-[10px] font-bold text-slate-600"
                          >
                            {s.size}
                          </span>
                        ))}
        ...
        <div className="p-4 bg-slate-50/40 border-t border-slate-100 flex items-center justify-between text-xs font-semibold text-slate-500">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              disabled={page === 1}
              className="p-1.5 rounded-lg border border-slate-200 hover:bg-slate-100 transition-colors disabled:opacity-50"
            >
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
              <button
                key={p}
                onClick={() => setPage(p)}
                className={`px-3 py-1 rounded-lg transition-all ${page === p ? "bg-brand-green text-black font-bold" : "border border-slate-200 hover:bg-slate-100"}`}
              >
                {p}
              </button>
            ))}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 text-xs rounded-xl focus:outline-none focus:border-brand-green"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 text-xs rounded-xl focus:outline-none focus:border-brand-green h-20 resize-none"
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 text-xs rounded-xl focus:outline-none"
                  >
                    {categoryOptions.map((c) => (
                      <option key={c} value={c}>
                        {c}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <label
                    ...
                    className="min-h-32 border-2 border-dashed border-slate-200 rounded-2xl p-4 bg-slate-50 flex flex-col items-center justify-center text-center hover:bg-slate-100 hover:border-brand-green transition-all cursor-pointer"
                  >
                    {imagePreview ? (
                      <img
                        src={imagePreview}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">
                    SVG de Probador Virtual
                  </label>
                  <label
                    htmlFor="product-svg"
                    className="min-h-32 border-2 border-dashed border-brand-green/60 rounded-2xl p-4 bg-brand-green/5 flex flex-col items-center justify-center text-center hover:bg-brand-green/10 transition-all cursor-pointer"
                  >
                  ...
                  <input
                    ...
                    className="sr-only"
                    onChange={(e) =>
                      setSelectedSvg(e.target.files?.[0] || null)
                    }
                  />
                </div>
              </div>
                ...
                <div className="flex gap-2">
                  {sizeOptions.map((s) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${formData.sizes.includes(s) ? "bg-brand-green/10 border-brand-green text-brand-green" : "border-slate-200 text-slate-500 hover:border-slate-300"}`}
                    >
                      {s}
                    </button>
                  ))}
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  className="px-5 py-2.5 bg-brand-green hover:bg-brand-green-hover text-black text-xs font-bold rounded-xl transition-all"
                >
                  {editingProductId
                    ? "Actualizar Producto"
                    : "Guardar Producto"}
