...
export default function AdminCustomers() {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div>
        <h1 className="text-2xl font-black tracking-tight text-slate-900">Gestion de Clientes</h1>
        <p className="text-xs font-medium text-slate-500 mt-0.5">Visualiza y gestiona las cuentas de clientes.</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
          <span className="text-[10px] font-bold tracking-wider uppercase text-slate-400">Total Registrados</span>
          <h3 className="text-2xl font-black text-slate-900 mt-1">{totalCustomers}</h3>
        </div>
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
          <span className="text-[10px] font-bold tracking-wider uppercase text-slate-400">Activo</span>
          <h3 className="text-2xl font-black text-emerald-500 mt-1">{activeCustomers}</h3>
        </div>
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
          <span className="text-[10px] font-bold tracking-wider uppercase text-slate-400">Bloqueado</span>
          <h3 className="text-2xl font-black text-rose-500 mt-1">{blockedCustomers}</h3>
        </div>
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
          <span className="text-[10px] font-bold tracking-wider uppercase text-slate-400">Administradores</span>
          <h3 className="text-2xl font-black text-blue-500 mt-1">{adminCustomers}</h3>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row items-center gap-3">
        <div className="relative flex-1 w-full">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por nombre, email..." className="w-full pl-9 pr-4 py-2.5 bg-slate-50 border border-slate-200 focus:border-brand-green text-xs rounded-xl focus:outline-none font-semibold" />
        </div>
        ...
        <select value={filterRole} onChange={(e) => setFilterRole(e.target.value)} className="bg-white border border-slate-200 text-slate-700 text-xs font-bold rounded-xl px-3 py-2.5 outline-none cursor-pointer">
                ...
                {filtered.map((c) => (
                  ...
                  >
                    <td className="py-4 px-4">
                      <div className={`w-9 h-9 rounded-full ${c.avatarColor} flex items-center justify-center text-white text-xs font-bold`}>
                        {c.initials}
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-6 font-extrabold text-slate-900">${c.totalSpent.toFixed(2)}</td>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-5 sticky top-8">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-14 h-14 rounded-full ${selectedCustomer.avatarColor} flex items-center justify-center text-white text-lg font-bold`}>
              ...
              <div className="space-y-3">
                <div>
                  <label className="text-[10px] font-bold tracking-wider uppercase text-slate-400 block mb-1">Estado de Cuenta</label>
                  <button
                  ...
                  >
                    {selectedCustomer.status === "Active" ? "Activo - Click para Bloquear" : "Bloqueado - Click para Activar"}
                  </button>
                </div>
                <div>
                  <label className="text-[10px] font-bold tracking-wider uppercase text-slate-400 block mb-1">Rol de Usuario</label>
                  <select
                    value={selectedCustomer.role}
                    onChange={(e) => handleRoleChange(selectedCustomer.id, e.target.value as CustomerRole)}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-50 rounded-xl p-3 text-center">
                  ...
                  <p className="text-lg font-extrabold text-slate-900 mt-1">${selectedCustomer.totalSpent.toFixed(2)}</p>
                </div>
                <div className="bg-slate-50 rounded-xl p-3 text-center">
                  <span className="text-[10px] font-bold tracking-wider uppercase text-slate-400">Pedidos</span>
                  <p className="text-lg font-extrabold text-slate-900 mt-1">{selectedCustomer.ordersCount}</p>
                </div>
              </div>

              {selectedCustomer.recentOrders.length > 0 && (
                <div>
                  <h4 className="text-[10px] font-bold tracking-wider uppercase text-slate-400 mb-2">Pedidos Recientes</h4>
